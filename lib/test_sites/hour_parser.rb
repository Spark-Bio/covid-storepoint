...
module TestSites
  class HourParser
    ...
    HS_CHECK_WEBSITE = HourSpecifier.new(*(0..6).map { 'check website' })
    HS_CALL = HourSpecifier.new(*(0..6).map { 'call for hours' })
    HS_24_7 = HourSpecifier.new(*(0..6).map { '24 hours' })
    HS_NOT_YET_AVAILABLE = HourSpecifier.new(*(0..6).map { 'not yet available' })
